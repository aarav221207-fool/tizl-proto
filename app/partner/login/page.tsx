...
function PartnerLoginContent() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (user?.role !== 'cook') {
        await logout();
        setError('This account is not registered as a Partner Cook. If you are a customer, please sign in at Customer Login.');
        return;
      }
      router.push(redirect);
    } catch (err: any) {
      setError(err.message || 'Failed to sign in to partner account');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleGoogleLogin = async () => {
    try {
      ...
      const { error: oauthError } = await supabase.auth.signInWithOAuth({
        provider: 'google',
        options: {
          redirectTo: `${window.location.origin}/api/auth/callback?next=/partner/dashboard&role=cook`,
        },
      });
      ...
      <main className="flex-1 flex flex-col justify-center py-12 px-4 sm:px-6 lg:px-8">
        <div className="sm:mx-auto sm:w-full sm:max-w-md">
          <h2 className="mt-6 text-center text-3xl font-bold text-[var(--ink)]">
            Partner Cook Sign In
          </h2>
          <p className="mt-2 text-center text-sm text-[var(--text-dim)]">
            New cook?{' '}
            <Link href="/partner/signup" className="font-semibold text-[var(--gold)] hover:underline">
              Apply to become a partner cook
            </Link>
          </p>
        </div>
        ...
        <div className="mt-8 sm:mx-auto sm:w-full sm:max-w-md">
          <div className="bg-white py-8 px-6 shadow-sm rounded-2xl border border-[var(--line)] sm:px-10">
            ...
            <div className="mt-6 text-center text-xs text-[var(--text-faint)]">
              Customer trying to book? <Link href="/customer/login" className="text-[var(--blue)] font-semibold hover:underline">Customer Login →</Link>
            </div>
          </div>
        </div>
      </main>
